    ...
    def parse_exam_paper(
        ...
        try:
            ...
            instruction_items = [
                ...
                "将题目内容转换为LaTeX格式，选择题选项使用enumerate环境，选择题的选项之间使用\\item命令，选项内部不需要再添加ABCD标签。表格使用table环境。除此之外不要使用其他环境，如\\centering等。空行直接插入空行，而不要使用\\par命令或\\\\来强制换行",
                "识别题目中引用的图片（如果有），从可用图片列表中选择合适的图片，严格返回可用的图片文件列表中的文件名，不要新增前缀或移除后缀。只能在image字段中声明所需要的图片，不要重复在question字段的原始题目中使用包含图片的\includegraphics命令。",
                "识别OCR文本中的表格结构（使用<table>、<tr>、<td>、<br>标签），移除这些标签并转换为标准的LaTeX table/tabular 环境，保持单元格内容。",
                "对于选择题，在合适的位置插入一对括号()来表明需要解答内容的位置，对于填空题，在合适的位置插入\underline{\hspace{1cm}}来表明需要解答内容的位置。",
            ]
